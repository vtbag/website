...
2. After the build completes successfully, run `node bin/rss-update.ts` to update the `public/rss.xml` file. Do not run this in parallel with the build. The script also updates `src/content/docs/recent-updates.md`. Leave this file as generated by the script—do not restore it to `HEAD` and do not manually edit it. Only `public/rss.xml` is reviewed and corrected manually. The script reassigns dates to all entries based on file modification times, even if the actual content changes are minor.
...
4. For each entry listed as CHANGED, use the source file path reported by `node .github/skills/update-rss/diff-rss.ts`. Check whether the change since the old `pubDate` is minor:
  ...
  - If that command returns no commit, stop treating it as a routine minor-change candidate. Check for missing commits or incomplete git history before proceeding. If unresolved, keep the new `pubDate`.
  ...
  - If uncertain, apply this test: if an experienced reader would keep the same implementation decisions after reading both versions, classify as **minor**. If they would learn a *new technique* or *new capability*, classify as **substantive**.
