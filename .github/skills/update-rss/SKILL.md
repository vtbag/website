...
2. After the build completes successfully, run `node bin/rss-update.ts` to update the `public/rss.xml` file. Do not run this in parallel with the build. The script also updates `src/content/docs/recent-updates.md`. Leave the .md file as generated by the script. Do not restore the .md file to `HEAD` and do not manually edit the .md file. Only `public/rss.xml` is reviewed and corrected manually. The script reassigns dates to all entries based on file modification times, even if the actual content changes are minor.
