...
And **Over-the-Top** is&hellip; well, very much over the top: _experimental_, _fancy_ and ...\
...**<a id="disclaimer"></a>Disclaimer: might not work at all in your settings.**
...
To experiment with the over-the-top flavour include the `@vtbag/element-crossing/experimental` and the `@vtbag/element-crossing` script in that order.
...
The `data-vtbag-x` attribute accepts a string that has to fulfil the following grammar:
...
Make sure to read the [disclaimer](#disclaimer) above. Over the Top is just a playground, not meant for production, and it may never get extended and polished enough for that.

The Over-the-Top flavour provides one additional expression for the `data-vtbag-x` attribute and an additional API similar to `window.sessionStorage`.
...
- Using the Over-the-Top flavoured Element Crossing can corrupt the remaining browser history for pages within its coverage area if you leave and then re-enter that area using the browser's back/forward navigation.
