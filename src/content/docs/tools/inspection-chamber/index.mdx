...
The Inspection Chamber requires a current **Chrome browser version 126 or higher**, and you must also manually activate **chrome://flags/#view-transition-on-navigation-iframe**  .
:::

The Inspection Chamber currently only works with the **Chrome browser, starting from version 126**. Additionally, you **must enable Chrome's _"viewTransition API for sub-frame navigations"_ feature**.
Otherwise, Chrome will not support view transitions inside iframes, and the Chamber will not work. To enable this feature, enter `chrome://flags/#view-transition-on-navigation-iframe` into Chrome's address bar and select `Enabled` from the drop-down menu.
