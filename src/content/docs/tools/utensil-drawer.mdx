...
...
const transition = document.mayStartViewTransition(...);
...
* `in-place` rewrites the selectors in place, 
* `append` appends the rewritten rules to the original stylesheet.   The following table shows how to combine the plugin and the polyfill parameters.

|When to use|useTypesPolyfill|postcss-active-view-transition-type mode|
|---|---|---|
|The recommended way: Activate polyfill only where needed and have original and rewritten rules in your CSS|'auto'| 'append'|
| If you do not want to use the polyfill|'never' | n.a.|
| You can avoid duplication of rules if you do not use types with cross-document view transition and all same-document view transition are started by `mayStartViewTransition()` with `useTypesPlugin: 'always'`|'always' | 'in-place'|
