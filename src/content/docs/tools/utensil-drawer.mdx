...
Here is an example from the [Derived Trajectories](/vector-demo/) demo: `setVector` is called right at the beginning of the animation phase when all old and new images are setup.
```js
startViewTransition(...).ready.then(()=>setVectors())
```
...
3. The View Transition API cancels active view transitions when a new one starts. `mayStartViewTransition` provides alternatives to prevent these cancellations. It can:
    ...
    * If animations are already in progress, the function queues new update  callbacks and runs them once the current view transitions finishes, combining them into an update function for a new view transition.
...
In the simplest use case, `mayStartViewTransition` works as a drop-in replacement for `document.startViewTransition`. Like the Level 2 API, it accepts either an update callback or an options object with a callback and view transition types.

When the View Transition API is not supported, it falls back gracefully by calling the update function directly and returning a compatible object with `updateCallbackDone`, `ready` and `finished` promises.

You can also pass a second parameter for advanced behavior not available in the native API:
