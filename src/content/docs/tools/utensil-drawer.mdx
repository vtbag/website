...
:::note
As the `setVectors()` function needs access to the [current view transition](#current-view-transition), it only works in combination with [mayStartViewTransition](#maystartviewtransition).
:::
...
### useTypesPolyfill: "always" | "never"

Setting `useTypesPolyfill: "always"` adds a **polyfill for [view transition types]()**.
This bring view transition types even to browsers that [only support Level 1](/basics/levels/#same-document-features-missing-in-level-1) of the View Transition API.
...
Setting `useTypesPolyfill: "never"` completely disable the polyfill. This is the default. 
