...
<UtensilGrid>
  ...
  <Utensil
    ...
    stable={false}
    description="Wrapper with a non-animating fallback for browsers without native support. Respects the reduced motion preference."
...
If `::view-transition-group(group)` morphs form _(x1, y1)_ to _(x2, y2)_, calling `setVectors()` adds the following CSS definitions for each view transition group:
```
:root {
...
```
This way you can access the parameters of the morph animations directly within you CSS rules. Other parameters like width and height might follow in the future.
