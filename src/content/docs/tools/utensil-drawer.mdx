...
The defaults are:
```js
{ respectReducedMotion = true, chaining = false, speedUpWhenChained = 1 }
```

With `respectReducedMotion`, `mayStartViewTransition` behaves as if native view transitions are not supported, if the user has set preference for reduced motion on the operation system or browser level.

When `chaining` is set to true, new transitions do not kill the current one but collect all the update callbacks and chain them in a new view transition once the current transition finishes. Hitting a current transition with a new call can also increase the speed of the current animation, if `speedUpWhenChained` is set to a value _> 1_.
