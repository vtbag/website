...
Assume `::view-transition-group(<group>)` having _width1_, _height1_ morphs form _(x1, y1)_, to _(x2, y2)_ having _width2_, _height2_. Calling `setVectors()` when the view transitions `ready` promise fulfils, adds the following CSS definitions to your page:
...
Like `collisionBehavior: "chaining"`, but not combining updates that arrive before the capturing. Updates arriving while a view transition is running are always queued.
...
This PostCSS plugin rewrites the pseudo-class selectors `:active-view-transition-type()` and `:active-view-transition` to selectors that work with the Bag's [polyfill for view transition types](/tools/utensil-drawer/#usetypespolyfill-always--auto--never). Plugin and polyfill together let you use
