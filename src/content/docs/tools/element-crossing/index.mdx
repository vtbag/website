...
<Tabs>
  <TabItem label="Using the npm package" icon="seti:npm">
    <Steps>
1. Install `@vtbag/element-crossing@latest` from npm.
...
<html>
  <head>
    <script src="https://cdn.jsdelivr.net/npm/@vtbag/element-crossing" />
    ...
  </head>
  ...
</html>
...
<html>
  <head>
    <script src="https://unpkg.com/@vtbag/element-crossing" />
    ...
  </head>
  ...
</html>
...
<svg xmlns="http://www.w3.org/2000/svg" width="583" height="217">
{/*
...
#### style
The style expression identifies a CSS property of the current HTML element. The value of this property is transferred to the same property on the target element on the next page.
...
* Currently, entries in sessionStorage are not removed after use, and there is no configuration option to control this behavior. As a result, memory usage increases during navigation until the session is closed. On the plus side, you can resume a video from where you left off, even if you've visited many other pages in the meantime.