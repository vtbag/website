...
The Turn-Signal package is a set of scripts for enhancement of cross-document view transitions. They automatically detect direction of navigation or let you set view transition types. The current tricks are
...
Check out the [Fishpond Demo](/link-demo/)! Glide smoothly into the water or take a bold dive.
...
```jsx
import signal from "@vtbag/turn-signal/directions?url";
<html>
  <head>
    <script src={signal} />
    ...
...
|---|---|---|
|directions|@vtbag/turn-signal/directions|lib/directions.js|
|link types|@vtbag/turn-signal/link-types|lib/link-types.js|
...
The `@vtbag/turn-signal/directions` script accepts several config options. Those are specified as `data-*` attributes on the `<script>` element. They are all optional.
