...
:::info
The Turn-Signal scripts are designed for use with cross-document view transitions. With the exception of the _forced_traversal_ script, all scripts will even work with none or only small limitations on browsers that do not support the Navigation API.
:::
...
<html>
  <head>
    <script src={signalDirections}></script>
    ...
  </head>
  ...
</html>;
...
Navigation API is baseline since late 2025. Older browsers might not support the Navigation API and will therefore not set the index properties in the `activation` object of the `pageswap` event. As a consequence, the turn-signal scripts can not determine types on the old page during history traversals (forward/backward navigation using browser buttons) and only set the `old` type. On the new page, types are set the same way as for all other browsers.
:::
