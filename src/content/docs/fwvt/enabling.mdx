...
These are the default settings the View Transition API applies when used without additional configuration:
...
- The animation is a cross fade: the old page fades out as its opacity goes from 1 to 0, while the new page fades in with its opacity going from 0 to 1.
