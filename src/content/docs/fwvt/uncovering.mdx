  ...
  <script is:inline type="speculationrules">{`
    {
      "prerender": [
        {
          "urls": ["/fwvt/playing/", "/fwvt/twinkling/"]
        }
      ]
    }
  `}</script>
...
But while the top-down reproduction kept the bug alive, oddly enough, the bottom-up project could not reproduce the bug.
...
![An paragraph overflowing to the right beyond the device limits](@/assets/fwvt/overflow.png)
...
The [minimal reproduction](https://fun-with-view-transitions.pages.dev/episode/6/page) is waiting for you.
