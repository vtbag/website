...
We can select and animate these pseudo-elements just like `<img>` or `<div>` elements. 

The default animation defined by the View Transition API is a cross-fade between the old and new image. It does a simultaneous fade-out of the old image and a fade-in of the new image. Getting those animations right [is a bit tricky](/tips/over-exposure/) as it also requires to set the right mix-blend-mode. 

So if you want a cross fade, just keep the original defaults. If you are curious about the details, [take a look at the user agent stylesheet](/basics/styling/#cross-fade-of-the-images).


The View Transition API sets the animation duration to 0.25s on the `::view-transition-group(root)`, which is [inherited](/basics/styling/#inherited-animation-properties) through the `::view-transition-image-pair(root)` down to the `::view-transition-old(root)` and `::view-transition-new(root)` pseudo-elements. This explains why we noted _"The animation lasts 250 milliseconds using CSS's default ease timing function."_ at the [end of Episode One](/fwvt/enabling/#reviewing-our-progress-so-far#:~:text=The%20animation%20lasts,function.).
...
* This also gives you first-hand **experience with one current limitation** of the View Transition API: although you can see the new page during the animation, you can't interact with it until the transition ends.
