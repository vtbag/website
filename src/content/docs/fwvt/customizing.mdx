...
We can select and animate these pseudo-elements just like `<img>` or `<div>` elements. For instance, we can fade out the old image using the following CSS rule:
...
Fortunately, we don't need to define this ourselves. This fade animation is already applied by the View Transition API through the browser's built-in user-agent stylesheet as the default behavior.

Well, not _exactly_ this, but very close. [See for yourself!](/basics/styling/#cross-fade-of-the-images)!
...
The animation duration of 0.25s is set on the `::view-transition-group(root)` and [inherited](/basics/styling/#inherited-animation-properties) through the `::view-transition-image-pair(root)`. This explains why we noted _"The animation lasts 250 milliseconds using CSS's default ease timing function."_ at the [end of Episode One](/fwvt/enabling/#reviewing-our-progress-so-far#:~:text=The%20animation%20lasts,function.).
