...
:::note[Edited]
Since I first published this episode, I made changes to the demo so it also works with Firefox.

1. `mayStartViewTransition()` is now called with  [`useTypePolyfill='always'`](/tools/utensil-drawer/#usetypespolyfill-always--auto--never), and the postcss-active-view-transition-type [PostCSS plugin](/tools/utensil-drawer/#the-postcss-active-view-transition-type-postcss-plugin)  was added to the project. This allows you to use view transition types even with browsers that do not yet support this Level 2 feature.

2. The `lift-and-shift` view transition type is only set [_after_ `setVectors()`]() ran. This prevents timing issues where browsers differ in the way they initialize `@keyframes` with custom-properties during view transitions. 
:::
...
No worries, you won't have to solve the puzzle yourself. It all runs automatically! But you can [play with it and explore its features](http://fun-with-view-transitions.pages.dev/episode/7/page).
...
You are curious about [`useTypesPolyfill: "always"`](/tools/utensil-drawer/#usetypespolyfill-always--auto--never)? This has to do with the view transition type pattern that I recommend to structure view transition CSS rules. It has its own section [further down the page](#scoping-with-active-types).
...
Setting the `lift-and-shift` view transition type after the vectors are set enables the `lift-and-shift` animation on all disks:
```css
...
### Avoid Shorthand Properties, Embrace Inheritance
...
Bonus perk: Thanks to the inheritance definitions, you only need one rule to change the duration of all animations in this demo:
...
The rules guarded with `:active-view-transition-type(some-type) {...}` are active while the type `some-type` is included in the view transition's types. 
...
:::note
Since I was so used to view transition types in Chrome, Edge, and Safari, I only recently realized that my beloved type guard pattern will not work once Firefox ships Level 1 view transitions.

So there will be Firefox versions that support same-page view transitions but no view transition types. And there are still older versions of the other browsers with the same constellation. The good news is: **The Bag provides a solution!**  

The [`useTypesPolyfill: 'always'`](/tools/utensil-drawer/#usetypespolyfill-always--auto--never) extension of the `mayStartViewTransition()` function provided by the Bag's `utensil-drawer`, together with the [`postcss-active-view-transition-types`](/tools/utensil-drawer/#the-postcss-active-view-transition-type-postcss-plugin) plugin, adds a polyfill that lets you use the `:active-view-transition-type()` pseudo-classes as if they were already built in.
:::

For more on type scoping pattern, check the [CSS Tips & Tricks](/tips/css/#scoping-with-types) section.
