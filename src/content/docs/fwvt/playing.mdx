...
```
You can use this to guard animations of the pseuo-elements, but you can even use it to _define_ the pseudo-elements by setting view transition names nested inside the type guarded rule. And of course you can use this pattern to style regular DOM elements _just for the time_ a certain view transition is active. 

For [more on this pattern](/tips/css/#scoping-with-types), check the CSS Tips & Tricks section.
