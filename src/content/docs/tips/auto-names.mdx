...
* For same-document view transitions, only: generate [dynamic identifiers](/basics/pseudos/#auto-generated-names-auto-and-match-element) with `match-element` (or `auto`): \
`selector { view-transition-name: match-element; } /* or auto */`
* _Not yet baseline: Copy an existing unique attribute like `id`: \
