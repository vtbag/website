...
Chrome offers [`view-transition-name: attr(<attribute-name> <type>?, <fallback-value>?)`](/basics/pseudos/#using-id-as-view-transition-name) as an alternative way to set view transition names.
Currently, this typed `attr()` function is _far from being baseline on all browsers_.


If you already have unique values in an attribute like `id`, and they are [valid identifiers for view transition names](/basics/pseudos/#escaping), `attr()` can copy those values as view transition names. This way, you get unique view transition names without having to come up with them yourself. 
...
Another benefit of this approach is that it offers more flexibility: Once the `id`s are in place, you can control when to use them as view transition names using your CSS selectors. For example, you could choose to set names only on certain list elements:
