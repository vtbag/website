...
* Currently only feasible for same-document view transitions: generate dynamic identifiers with [match-element](/basics/pseudos/#auto-generated-names-match-element)

* In rare cases also for cross-document view transitions: generate dynamic identifiers with [auto](/basics/pseudos/#auto-generated-names-auto) for elements with `id` properties.

* [Copy an existing unique attribute](/basics/pseudos/#auto-generated-names-match-element) like `id`
...
Where it is supported, it works for same-document view transitions.

Safari generates rather short names.
```cs
-ua-auto-2235
-ua-auto-2236
-ua-auto-2237
...
```
Chrome's dynamically generated names can technically distinguish the owner documents of the old and new page elements during cross-document transitions. 
...
Chrome offers [`view-transition-name: attr(id type(<custom-ident>))`](/basics/pseudos/#auto-generated-names-match-element) as an alternative way to set view transition names.
Currently, this typed `attr()` function is _only available in Chrome_.
