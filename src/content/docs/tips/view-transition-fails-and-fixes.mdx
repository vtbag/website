...
#### Potential Pitfall: Unreliable Detection of the Opt-in
  ...
  A particularly perfidious pitfall occurs when the `@view-transition` at-rule is present but not reliably detected by the browser. This can happen in Chrome, for example, when the at-rule is placed in an external stylesheet after tons of other CSS.
