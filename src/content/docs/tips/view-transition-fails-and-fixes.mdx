...
Let's start with the most annoying category of problems: you click a link, trigger a view transition, and nothing happens. No animation, just a normal navigation, maybe even without an error message. How do you find the first clue to the problem? And how do you fix it?
...
<Details name="fail" class="c">
  ...
  ```
    Failure to include the opt-in on the target side of a cross-document view transition might lead to a general, not very helpful error message:
  ```text del={1}
  Uncaught (in promise) AbortError: Transition was skipped
  ``` 
  
  It might seem best to place this rule together with your global view transition styles in a shared stylesheet and [include it on every page](/fwvt/enabling/#including-css-on-all-pages). This seems to be fine if the @view-transition opt-in is at the start of the stylesheet. But I recommend to place it in an `<style>` element within the `<head>`, see next section. 

##### Potential Pitfall: Unreliable Detection of @view-transition

  A particular pitfall occurs when the `@view-transition` at-rule is present but not reliably detected by the browser: Sometimes it works, sometimes it doesn't. This can happen in Chrome, for example, when the at-rule is placed in an external stylesheet after tons of other CSS.
  ...
  The safest approach is therefore to place the opt-in for cross-document view transitions inside a `<style>` element early within the `<head>`.
...
In the original design of the View Transition API, duplicate names raised an error and cancelled the view transition, as if it had been explicitly stopped by `skipTransition()`. Newer versions may simply ignore duplicate names.
...
<Details name="fail" class="s">
  ...
  Thus you can directly follow up with another call to `startViewTransition()`.
...
If you call `skipTransition()` before the [ready promise](/basics/javascript/#promises) fulfils, the view transition API will not insert the `::view-transition` pseudo-element tree and the animations won't start at all.
...
Now you have a view transition, but it does not look as you expected. Maybe some animations are missing, or the stacking order is wrong. Here are some common reasons for that and how to fix them.
...
The reason could be that the browser starts view transitions too early during page load. If that is the case, the browser needs a bit of help to decide on the right moment. Use [`blocking="render"` attributes](/basics/pseudos/#delaying-cross-document-transitions), to tell the browser what part of the page to wait for before the transition should start.
...
```
Note how scoping changed the view transition class name in the second selector. We now have an unused view transition class named `my-box` and an undefined view transition class named `_my-box_1p56a_15`.
...
Ever had the feeling that your view transition just hangs and does not end? Here are some common reasons for that.
