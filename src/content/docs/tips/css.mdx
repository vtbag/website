...
Once Firefox adds native support for Level 1 view transitions, this will likely break [because `:active-view-transition-type` is only available with Level 2 of the API](/basics/levels/).

I'm actively working on a polyfill to bring view transition types to browsers that only support Level 1 view transitions. Stay tuned!
:::
