...
When you start to trigger different view transitions on the same page, you will notice that it is rather easy to end up with conflicting defintions. 
...
What you need is a mechnism to isolate the definitions for different use cases. View transition types are a great tool for that. You can use them to guard the CSS definitions for each use case, so they only apply during the view transition of that specific use case.
