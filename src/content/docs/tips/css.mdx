...
Once Firefox adds native support for Level 1 view transitions, this will likely break because `:active-view-transition-type` is only available with Level 2 of the API.

As an alternative to view transition types, you can set CSS classes with the same name on the `:root` element and replace `:active-view-transition-type(some-type) {...}` with `:root.some-type {...}`.

Keep in mind you cannot combine both selectors. Browsers that do not understand the `:active-view-transition-type` syntax will also ignore a combined selector like `:active-view-transition-type(some-type), :root.some-type`.
:::
I am actively exploring this and will update you once there is more to share.
