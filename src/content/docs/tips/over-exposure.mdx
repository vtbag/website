...
Additionally, the browser's built-in stylesheet automatically assigns the `isolation` CSS property with the value `isolate` to all image pairs with both images. This ensures that blending is confined to the images themselves, unaffected by the background.

The `-ua-mix-blend-mode-plus-lighter` animation is only added by the browser if the image-pair has both, an old and a new image. Single children (:only-child) in image pairs blend in normal mode.

When you define an animation that fades the images but does not overlap them, e.g. by moving and fading the old image out of the viewport while the new image slides and fades in, you might want to reconsider the `plus-lighter` setting, as it can lead to unexpected visual results. In such cases, you can cancel the plus-lighter mode by explicitly omitting `-ua-mix-blend-mode-plus-lighter` in the `animation-name` value. Or you simply define your own keyframes for your `slide-and-fade` animation without `mix-blend-mode` or with `mix-blend-mode: normal`.
