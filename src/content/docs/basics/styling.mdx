...
With the [`:active-view-transition`](#active-view-transition) pseudo-class, you can apply styles for the duration of a view transition even to DOM elements that are not created by the View Transition API. Keep in mind that those elements are typically [hidden behind the view transition pseudo-elements](/basics/pseudos/#the-theatre-curtain) during a view transition.
...
::view-transition-old(x) {
  animation-name:
    ...
    -ua-mix-blend-mode-plus-lighter; /* only if the image pair has both images */
}
::view-transition-new(x) {
  animation-name:
...
@keyframes -ua-view-transition-fade-out {
  /* implicitly: from {opacity: 1}, if not set otherwise*/
  to {
    opacity: 0;
  }
}
@keyframes -ua-view-transition-fade-in {
  ...
  /* implicitly: to {opacity: 1}, if not set otherwise*/
}
