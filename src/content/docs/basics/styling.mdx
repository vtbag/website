...
The browser automatically assigns the view transition name `root` to the `:root` element, i.e. the top-level `<html>` element of the DOM. If you prefer another name, you can override it. If you do not want the `<html>` element to participate in the view transition with own pseudo-elements, you explicitly have to remove the `root` name with either
```css
:root {
...
But when you assign to shorthand properties, most important `animation`, you override the definitions from the user agent stylesheet. And if you do only specify some values for the shorthand, you have to know browser defaults for the values you do not specify explicitly.
...
Here is what the API automatically assign in its user agent style sheet for a view transition name `x`. In keyframe names, `-ua` stands for _user agent_.
