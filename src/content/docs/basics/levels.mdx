...
- no support for the option parameter when calling `document.startViewTransition()`
...
Firefox , starting with v144, not only supports Level 1 of the View Transition API, but also brings some Level 2 features:
* Firefox already [has support for `view-transition-class`](https://bsky.app/profile/vtbag.dev/post/3ltwvggp7xc2u) and the [`:active-view-transition` pseudo-class](/basics/test-page/).
* Firefox already [has support for `view-transition-name: match-element`](https://bsky.app/profile/martr.app/post/3lwyzve7nac2i), but not for `view-transition-name: auto`, which is not really part of the standard. When usings the [declarative-names script](/tools/utensil-drawer/#declarative-names) from The Bag's Utensil Drawer, you also have an alternative for dynamically generating view transition names that works more reliably across browsers.
* In the meanwhile, Firefox also supports `:active-view-transition-type()`.
* So the main problem might be the missing support for view transition types in older versions. Good news here: The Bag's Utensil Drawer now has a [polyfill for view transition types](/tools/utensil-drawer/#usetypespolyfill-always--auto--never) to support them even on browsers that only implement Level 1 of the View Transition API!
* And even if you do not care about types yet, you can still benefit from the fact that [mayStartViewTransition](/tools/utensil-drawer/#maystartviewtransition) also offers the Level 2 signature with options, even if the underlying API implementation does not. 
