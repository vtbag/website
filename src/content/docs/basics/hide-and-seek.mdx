...
_Scoped_ view transitions are still a bit further in the future. Once available, they will also enable clipping view transition animations. 
They will even enable two different approaches. 

The view transition pseudo elements can be clipped at containers outside the view transition root. In or example, we could run a scoped view transition on the (overflowing) `<ul>` element and put a container with `overflow:scroll` around it, which we use it to clip the pseudo elements so they could not escape. 
...
Alternatively, we can opt out of self-participating by removing the `root` view transition name for `<ul>` and clip at the root pseudo element of the view transition:
```css
ul {
