...
### Clipping with **Nested** View Transition Groups
...
In the early versions of the View Transition API, the transition groups form a flat list of children of the `::view-transition` pseudo element. [_Nested_ view transition groups](/basics/pseudos/#nested-view-transition-groups) are already defined in the [Level 2 spec](/basics/api/#w3c-drafts-preliminary-specs), but are not yet widely supported by browsers.
...
```css
    ::view-transition-group-children(ul) {
      overflow: clip;
      width: calc(100% - 18px); // quick fix for https://issues.chromium.org/issues/475236700
    }
```

<a id="clipping-with-scoped-view-transition-groups"></a>
### Clipping with **Scoped** View Transitions
...
_Element-scoped_ view transitions, or _scoped_ view transitions for short, are currently supported in Chromium browsers, i.e. Chrome and Edge. Safari and Firefox have not yet landed support.

Scoped view transitions enable two different approaches to clipping list elements.

The view transition pseudo-elements can be clipped at containers outside the view transition root. In our example, we could run a scoped view transition on the (overflowing) `<ul>` element and place a container with `overflow: scroll` around it, which clips the pseudo-elements so they cannot escape.
...
Alternatively, we can opt out of self-participation by removing the `root` view transition name for `<ul>` and then clip at the root pseudo-element of the view transition:
```css
ul {
...
Until _nested_ view transition groups and _scoped_ view transitions are supported by all major browsers, we have to play some tricks to simulate the clipping. See the next section on how to do that.
...
<SidePicture>
It might sound a bit weird, to fight unwanted images with even more images, but the idea is simple: put an [additional curtain](/basics/pseudos/#the-theatre-curtain) in front of your stage to hide what you don't want the audience to see.
...
Using the stencil approach on the [original list example](/basics/hide-and-seek/problem/) from the beginning of this article now yields a [much better result](/basics/hide-and-seek/solution/).
