...
While I add more pages, I'm also happy that I can offer some links to previous work on the [`astro-vtbot` package](https://www.npmjs.com/package/astro-vtbot), especially an in depth [same-document example](https://events-3bg.pages.dev/jotter/api/example/)
...
#### Recent Changes
Test whether your browser supports the latest features of the View Transition API on [this test page](/basics/test-page/).

|When|What|Where|
|---|---|---|
|2026-06-29|You cannot use `none` or identifiers starting with `-ua-` as names for view transition types. If you try to add those to the view transition's types set, they will simply be ignored. The latter is consistent with the restriction that view transition names starting with `-ua-` are reserved for use by the browser (the user agent).|[View Transition Types](https://drafts.csswg.org/css-view-transitions-2/#types-cross-doc)|
