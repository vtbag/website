...
```
Here the definition of the view transition name creates a view transition group `my-name`. adding the `view-transition-group` property makes the new group a child of the `other` group defined elsewhere.
...
```
If the fragment used in the `href` attribute does not exist on the page, view transitions will wait for the whole page to load before they start. But be aware that the browser might log an error message in this case and it might cancel the view transition anyhow if waiting for the entire page takes too long.

Try to target the `href` to some HTML element that ends just "_below the fold_" to avoid killing incremental load of the rest of you page.
...
Prefetching or preloading resources can greatly reduce the wait time. At least on Chromium browsers speculation rules can be used for preloading.