...
Even if it were widely supported, would would not recommend `auto` right now for cross-document view transitions. There are [alternatives](/tips/auto-names/#how-to-assign-dynamic-names) that work cross-browser and also for cross-document navigation.
