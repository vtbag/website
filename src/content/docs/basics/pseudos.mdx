...
While theatre can create magic, view transitions go even further with their "curtains." These curtains not only move independently, but they can also change in size, orientation, opacity, and more. Some curtains even act as dynamic surfaces, projecting constantly changing content.
...
Capturing of images happens in two passes. One captures images of the old DOM state before the view transition, and the other pass captures the new elements of the target state.

For both passes, when the View Transition API captures the images to be animated, the **view transition names have to be unique** in the DOM. Otherwise, the API will skip the entire view transition, or in newer versions [drop the non-unique elements](/basics/api/#recent-changes).

Under certain circumstances, the API ignores view transition names on elements. When the view transition name of an element is ignored, it can not conflict with another element with the same name.

There are two main cases where the View Transition API ignores view transition names:

1. The API ignores the `view-transition-name` of a fragmented element, i.e. an element that consists of two or more boxes, like an inline element that spans two lines. View transition names assigned to such elements are ignored.
...
### Rendering Effects of View Transition Names

Mainly for feasibility and performance reasons, setting a view transition name on an element has some [lesser-known effects on rendering](https://drafts.csswg.org/css-view-transitions-1/#named-and-transitioning) of that element and its subtree. 

The most important one is that each element with a valid view transition name **creates its own stacking context**. This changes the ways in which you can order elements for rendering.  

Additionally, all child elements are flattened in 3D space, similar to `transform-style: flat`.

Setting a view transition name on an element also establishes a backdrop root for the element. 
...
Those names typically look like `-ua-....` [with some random suffix](/tips/auto-names/#generated-names). 
...
Because view transition names must be unique inside their DOM, there can be at most one old image and one new image with a given name. The API assumes that elements with the same `view-transition-name` on both the old and the new view represent the same logical concept. It is not necessary for these elements to be implemented the same way or even use the same HTML tags. The old and new images form a pair, and this pair is the only child of the group. Both `::view-transition-group` and `::view-transition-image-pair` are named using the `view-transition-name`. All groups are direct children of the `::view-transition` pseudo-element. Thus, a typical structure looks like this, where a pair can have one or two children.
...
With the one to one relation between groups and image pairs, the group looks a bit redundant. This 1:1 relationship changes with nested view transition groups. Without nesting, the view transition groups form a simple sequence and the order of the elements in this list also defines the standard paint order of the pseudos.

Nested view transition groups  add the possibility to arrange view transition groups in trees. Nested view transition groups are especially helpful as they allow [clipping of child groups](/basics/hide-and-seek/#clipping-with-nested-view-transition-groups).

The nesting of groups is implemented using a `view-transition-group-children` pseudo-element. It exists only in groups that really have children.
...
Nested view transition groups and the `::view-transition-group-children` pseudo-element are a relatively new features. Check whether your browser already [supports](/basics/test-page/) it.


Historical note: In browsers that support nested groups but do not yet support `::view-transition-group-children`, nested groups are direct children of the parent `::view-transition-group`, and they may appear before the image-pair.

The `::view-transition-group-children` pseudo-element appears after the parent group's image-pair (if any) and becomes the direct parent of all nested groups. This pseudo-element is only inserted for groups where the `view-transition-group` property is set. The name in parentheses for a group children pseudo-element matches that of its group.




When it creates a `::view-transition-group-children` pseudo element, the View Transition API also adds an [animation that morphs the border values](/basics/styling/#group-children-animation) of the old image to those of the new image.
...
Here the definition of the view transition name creates a view transition group `my-name`. Adding the `view-transition-group` property makes the `my-name` group a child of the `other` group defined elsewhere. You can not reference arbitrary groups, only those created for ancestors of the current element: A parent-child relationship in the view transition tree can only exist if the named elements also are descendants in the DOM. If the name is not found (for an ancestor), the new group will be inserted as a child of the `::view-transition` pseudo-element. 

##### view-transition-group: none
You can force a new group to be a child of the `::view-transition` root by setting `view-transition-group: none`.

##### view-transition-group: normal

If an element has `view-transition-group: normal`, a newly created view transition group for that element will be inserted as a child of the nearest view transition group that itself was created from an element with a `view-transition-group` value other than `normal`. In other words: the parent view transition group is determined by visiting parent DOM elements until an element is found that has a `view-transition-name !== none` and a `view-transition-group` value that is either `nearest`, `contain`, or some group name. If no parent group is found the new group will become a child of the `::view-transition` pseudo-element.

##### view-transition-group: nearest

The `nearest` value has the similar effect as normal, but a bit simpler. The difference is that during the traversal of the parent elements in the DOM the view transition group of the element that creates one will be used as parent group. Again, if no such group is found, the `::view-transition` pseudo-element will be the parent of the group.

##### view-transition-group: contain
For identifying a parent for the new group, `contain`  works the same way as `normal`. Setting `view-transition-group: contain` is mainly a handy shortcut to make a view transition group the automatic parent of all groups created by elements that are descendants of this element and do explicitly set their view-transition group.



## Creation of Pseudo-Elements

The `::view-transition-old` pseudo elements are created **in paint-order** of the elements of the old DOM. When an element with a `view-transition-name` is encountered, its old image is captured. During this process, other elements with view transition names are effectively ignored, as if their `visibility` were set to `hidden`. This ensures that the images have "holes" where other images can be properly placed.
...
The first pseudo-element that is rendered for a view transition is `::view-transition`. For a global view transition, it has the size of the viewport including scrollbars, and retractable UI parts on mobile. For a scoped view transition, it has the position and size of the view transition root. The spec refers to this as the _snapshot containing block_. The `::view-transition` pseudo-element is fully transparent by default. It works like a glass pane that prevents the user from interacting with elements below it (as long as you don't set `pointer-events: none` on it).
...
There is a clear separation between regular DOM elements and the view transition pseudo-elements. The pseudo-elements form their own stacking context, known as the _view transition layer_. This one is special as it sits in front of all other elements of the page. No regular page elements can be forced into or above the view transition layer, not even with high `z-index` values. For scoped view transitions, this is only true for the elements inside the view transition root, but not for elements outside of it: here you can have regular page elements that are rendered in front of the view transition layer of a scoped view transition.
...
With cross-document transitions the browser has to guess what would be a good time to start the animations of the view transition. It needs to load at least the HTML with the elements that should participate in a view transition. And it definitively needs to load the stylesheets that describe the view transition. But it would be a bad user experience if it would wait for a long page to load completely and await all scripts to run before the transition starts.
