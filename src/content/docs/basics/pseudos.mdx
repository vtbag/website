...
[Check if your current browser already supports it](/basics/test-page/). Auto copies the name of the `id` attribute if defined. Otherwise, it makes up some name like `-ua-auto-....`.

Using `auto` with `id` can be used to define elements with the same view transition name for cross-document navigation. But automatically generated names will always be different for two documents.
