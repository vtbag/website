...
On the HTML side, you can hide elements by adding the `hidden` attribute or by putting them inside a closed details element.
...
Here `view-transition-scope` is a relatively new CSS property that is currently only supported in Chrome. With value `all` it hides the view transition names of its subtree but does not change the rendering of the elements. 
