...
Nested transition groups introduce the ability to clip images not only within their own group but also using their parent or any ancestor further up the hierarchy. The example to the right is built with nested transition groups. When you press the button, and thi demo behaves the same as the one in the next section, your browser does not support nested transition group, yet.
