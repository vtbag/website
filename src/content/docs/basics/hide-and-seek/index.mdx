...
Clipping the list elements at the list container prevents them from being drawn outside the list element. However, if clipped at the parent group, they seem to fly in front of the scrollbars and the border.

Clipping at the `::view-transition-group-children` element respects the border. If you make it some pixels smaller, you can keep the scrollbar visible during the animation.

### Clipping with **Scoped** View Transition Groups
...
Scoped transitions also arrived recently in Chrome Canary 136. There seem to be issues when included in an iframe. If the example to the left seems to be broken, here is a [direct link](/basics/hide-and-seek/list-scoped/).
