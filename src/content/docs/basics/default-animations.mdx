...
### Corollaries
The captured images for the automatically added `::view-transition-group(root)` on the `<html>` element both have the [size of the viewport](/basics/pseudos/#important-exception). The group animation morphs the viewport into an area of same position and size. Thus only the cross-fade animation is observable.
