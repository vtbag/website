...
Astro, being a framework for multi page apps,  Astro's `<ClientRouter />` component (formerly known as `<ViewTransitions />`) supports **cross-document** view transitions. Astro's view transition support was the first in any web framework, predating React's by at least 18 month.
